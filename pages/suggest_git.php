...
// Two-line git commit format
$summaryLine = "{$task['step_number']} - " . ucfirst(trim($task['description']));
$detailsLine = "Implements: {$task['description']} under '{$task['phase_name']}' phase of '{$task['project_name']}'.";
...
<head>
  ...
  <style>
    ...
    code { font-family: monospace; font-size: 0.95rem; white-space: pre-wrap; }
  </style>
</head>
<body>
...
git commit -m "<?= htmlspecialchars($summaryLine) ?>" -m "<?= htmlspecialchars($detailsLine) ?>"</code></pre>
